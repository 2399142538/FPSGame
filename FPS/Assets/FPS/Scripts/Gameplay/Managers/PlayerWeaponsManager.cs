...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        [Header("References")] [Header("用于避免看到武器投掷几何形状的辅助摄像头")]
        public Camera WeaponCamera;

        [Header("父变换，所有武器都将添加到层次结构中")]
        public Transform WeaponParentSocket;

        [Header("武器处于活动状态但未主动瞄准时的位置")]
        ...
        [Header("武器鲍勃")]
        [Header("当玩家移动时，武器在屏幕上移动的频率")]
        public float BobFrequency = 10f;

        [Header("武器摆锤的使用速度有多快，值越大，速度越快")]
        ...
        [Header("武器反冲")]
        [Header("这将影响后坐力移动武器的速度，值越大，速度越快")]
        public float RecoilSharpness = 50f;

        [Header("后坐力可以影响武器的最大距离")]
        public float MaxRecoilDistance = 0.5f;

        [Header("反冲结束后，武器返回原始位置的速度有多快")]
        public float RecoilRestitutionSharpness = 10f;

        [Header("Misc")] [Header("播放瞄准动画的速度")]
        public float AimingAnimationSpeed = 10f;

        [Header("不瞄准时的视野")]
        public float DefaultFov = 60f;

        [Header("应用于武器相机的常规视场部分")]
        public float WeaponFovMultiplier = 1f;

        [Header("在第二次切换武器之前延迟，以避免从鼠标滚轮接收多个输入")]
        public float WeaponSwitchDelay = 1f;

        [Header("将FPS武器游戏对象设置为的图层")]
